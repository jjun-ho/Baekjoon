...
for i in range(n):
    for j in range(n - 1):
        ...
        if array[i][j] != array[i][j + 1]:
            array[i][j], array[i][j + 1] = array[i][j + 1], array[i][j]   # 좌우
            width()
            height()
            array[i][j + 1], array[i][j] = array[i][j], array[i][j + 1]  # 다음 반복을 위해 원래 상태 복귀
        # 만약 입력 받은 배열의 열의 원소가 다르다면
        if array[j][i] != array[j + 1][i]:
            array[j][i], array[j + 1][i] = array[j + 1][i], array[j][i]  # 상하
            width()
            height()
            array[j + 1][i], array[j][i] = array[j][i], array[j + 1][i]  # 다음 반복을 위해 원래 상태 복귀
